...
PATCH TARGETS (v0.8.0):
- HA functions: trapninja.processing.packet_handler.modules.ha.*
- forward_packet: trapninja.processing.packet_handler.forward_packet
- parse_snmp_packet: trapninja.processing.packet_handler.parse_snmp_packet
...
class TestPathSelection:
    ...
    def test_snmpv1_uses_slow_path(self, packet_queue, stop_event, mock_config):
        ...
        # parse_snmp_packet is imported at top of packet_handler, so patch there
        # get_enterprise_oid is imported inside function, so patch at parser level
        with patch.object(_config_cache, 'get', return_value=mock_config), \
             patch('trapninja.processing.packet_handler.forward_packet') as mock_forward, \
             patch('trapninja.processing.packet_handler.modules.ha.is_forwarding_enabled', return_value=True), \
             patch('trapninja.processing.packet_handler.modules.ha.notify_trap_processed'), \
             patch('trapninja.processing.packet_handler.parse_snmp_packet', return_value=(MagicMock(), "v1")), \
...
class TestHAStateIntegration:
    ...
    def test_secondary_node_blocks_forwarding(self, packet_queue, stop_event, mock_config):
        """Secondary node (forwarding disabled) blocks packet forwarding."""
        from trapninja.processing.worker import PacketWorker
        from trapninja.processing.worker import _config_cache
        ...
        with patch.object(_config_cache, 'get', return_value=mock_config), \
            ...
            # Should NOT forward when HA blocks
            mock_forward.assert_not_called()
